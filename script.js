                        ...
                        <div onclick="document.getElementById('event-${e.id}').scrollIntoView({behavior:'smooth', block:'start'})" 
                            ...
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                <div class="flex flex-col w-16 shrink-0">
                                    ...
                                    <span class="text-[7px] font-bold uppercase opacity-40 whitespace-nowrap mt-0.5 leading-none">${formatDisplayDate(e.event_date, e.formatted_date_display)}</span>
                                </div>
        ...
        const formatDisplayDate = (dateStr, displayStr) => {
            if (displayStr && displayStr.length > 10 && (displayStr.includes(' ') || displayStr.includes('th') || displayStr.includes('st') || displayStr.includes('nd') || displayStr.includes('rd'))) {
                return displayStr;
            }
            // If displayStr is missing or just an ISO date, format it nicely
            try {
                const d = new Date(dateStr);
                if (isNaN(d.getTime())) return displayStr || dateStr;
                    ...
                    <div id="event-${e.id}" data-date="${e.event_date}" class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm scroll-mt-24 transition-all ${isPast ? 'opacity-50 grayscale-[0.5]' : ''}">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-mono font-bold text-gray-400">${formatDisplayDate(e.event_date, e.formatted_date_display)}</span>
                            </div>
                        </div>
